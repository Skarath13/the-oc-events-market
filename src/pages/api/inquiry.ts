...
import { type FormResponse, formDataToObject, inquirySchema, zodErrors } from '@/lib/forms';
import {
  createCorrelationId,
  deliverForm,
  isRateLimited,
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (contentLength > 32_768) {
    return json(
      {
        ok: false,
        message: 'The submission is too large. Shorten the description and try again.',
        correlationId,
      },
      413,
    );
  ...
  const result = inquirySchema.safeParse(formDataToObject(formData));
  if (!result.success) {
    return json(
      {
        ok: false,
  ...
  if (result.data.website) {
    return json(
      { ok: true, message: 'Thank you — your inquiry was received.', correlationId },
      200,
    );
  }
  ...
  const delivery = await deliverForm({ kind: 'inquiry', submission: result.data, correlationId });
    ...
    return json(
      {
        ...
        message:
          delivery.reason === 'disabled' || delivery.reason === 'configuration'
            ...
            : 'The inquiry could not be delivered. Your information was not sent; please try again.',
        correlationId,
      },
