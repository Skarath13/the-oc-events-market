...
- Approved privacy notice and terms from the owner/legal reviewer.
...
- Optional public email, if the owner wants an email contact path later.
- The verified text number is linked behind text CTAs and printed only in the footer.
- Text messaging is the configured public contact method.
...
`showPhone` is `true` for the verified footer number. The remaining flags begin `false`:
`showPricing`, `showTestimonials`, `showTeam`, `showAddress`, `showVendorLogos`, `showAwards`,
`showJournal`, `showCelebrationStories`, `showProposalPlanning`, and
`showPackagePagesInNavigation`.
